...
describe("content", () => {
  ...
  describe("getAllTopics", () => {
    it("returns empty array when content directory does not exist", () => {
      const topics = getAllTopics(contentDir);
      expect(topics).toEqual([]);
    });

    it("returns topics from MDX files with frontmatter", () => {
      const sectionDir = path.join(contentDir, "guides");
...
# Hello`
    ...
    it("uses slug as title when frontmatter title is missing", () => {
      const sectionDir = path.join(contentDir, "tts");
      fs.mkdirSync(sectionDir, { recursive: true });
  ...
  describe("getTopicBySlug", () => {
    it("returns null for non-existent slug", async () => {
      ...
      const result = await getTopicBySlug("guides/nonexistent", contentDir);
...
Hello **world**`
      ...
      const result = await getTopicBySlug("guides/intro", contentDir);
    ...
    it("falls back to .md when .mdx does not exist", async () => {
      ...
      const result = await getTopicBySlug("reference/api", contentDir);
      expect(result).not.toBeNull();
    ...
    it("prefers .mdx over .md when both exist", async () => {
      ...
      const result = await getTopicBySlug("guides/dual", contentDir);
      expect(result).not.toBeNull();
      expect(result!.topic.title).toBe("MDX Version");
    });

    it("rejects path traversal slugs with '..'", async () => {
      fs.mkdirSync(contentDir, { recursive: true });
      const result = await getTopicBySlug("../../../etc/passwd", contentDir);
      expect(result).toBeNull();
    });

    it("rejects slugs with leading '/'", async () => {
      fs.mkdirSync(contentDir, { recursive: true });
      const result = await getTopicBySlug("/etc/passwd", contentDir);
      expect(result).toBeNull();
    });
    ...
    it("rejects slugs with backslashes", async () => {
      fs.mkdirSync(contentDir, { recursive: true });
      const result = await getTopicBySlug("guides\\..\\..\\etc\\passwd", contentDir);
      expect(result).toBeNull();
    });
  });
});
